...
public class FoldableListLayout extends FrameLayout {
		...
		gestureDetector = new GestureDetector(context, new SimpleOnGestureListener() {
			...
			public boolean onScroll(MotionEvent e1, MotionEvent e2, float distX, float distY) {
				if (mScrollCallback != null) {
					mScrollCallback.startedScrolling();
				}
	...
	public void scrollToPosition(int index) {
		...
		//als het kaartje toevalt op pos 0 zal de muziek stoppen
		if (index == 0 && mScrollCallback != null) {
			mScrollCallback.stoppedScrollingAndFirstIsVisible();
		}
	}
	...
	private class FlingAnimation implements Runnable {
		...
		private boolean mPerformCallback;
		...
		void stop() {
			...
			if (mPerformCallback && mScrollCallback != null) {
				mScrollCallback.stoppedScrollingAndFirstIsVisible();
				mPerformCallback = false;
			}
		}
		...
		boolean fling(float velocity) {
			...
			//als de kaart vanzelf op een positie beland door de start van een swipe, en neerkomt op de eerste kaart waardoor de card eigenlijk toe is, dan moet de muziek stoppen
			mPerformCallback = velocity < 0 && position == 0;
